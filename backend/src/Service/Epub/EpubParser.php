...
class EpubParser {
    ...
    private function _getTOC() {
        ...
        $tocFileHref = $this->getFileRealPath('toc.ncx');
        if (is_array($tocFile) && isset($tocFile['href'])) {
            $tocFileHref = $tocFile['href'];
        } else {
            $tocResults = $this->getManifestByHref('toc.ncx');
            if (count($tocResults) > 0) {
                $tocFileHref = reset($tocResults)['href'];
            } else {
                $ncxResults = $this->getManifestByHref('/\.ncx$/');
                if (count($ncxResults) > 0) {
                    $tocFileHref = reset($ncxResults)['href'];
                }
            }
        }

        $buf = $this->_getFileContentFromZipArchive($tocFileHref);
    ...
    private function _getFileContentFromZipArchive($fileName) {
        ...
        if (!$fp) {
            throw new \Exception("Error: can't get stream to epub file: {$this->zipArchive->getStatusString()}");
        }
    ...
    public function getManifestByHref($pattern) {
        $isRegExp =  @preg_match($pattern, '') !== FALSE;
        return array_filter($this->manifest, function($manifest) use($pattern, $isRegExp) {
            if ($isRegExp) {
                return preg_match($pattern, $manifest['href']);
            }
            return $manifest['href'] === $pattern;
        });
    }
...
//                return $matches[1].$this->imageWebRoot.'/'.$img.$matches[3];
    ...
    public function getChapterByHref(string $refItemHref)
    {
        $ncxResults = $this->getManifestByHref('/'.$refItemHref.'$/');

        if (count($ncxResults) > 0) {
            $chapterId = array_keys($ncxResults)[0];
